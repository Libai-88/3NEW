...
口径约定
  ...
  · 附件A(环氧)只提取 T弯：T弯G 腐蚀判定折算为近似 mm 当量并纳入 T弯 标签
      （0腐蚀≈5、<10mm腐蚀≈7、10mm腐蚀≈12、15mm腐蚀≈18、>15mm腐蚀≈22；
        点状/有改善 等无法量化者保留为空）。
...
# 附件A T弯G 腐蚀判定 -> 近似 mm 当量（低 mm = 通过更严苛的弯曲，性能更好）
TWG_MM = {
    '0腐蚀': 5.0, '0': 5.0,
    '<10mm腐蚀': 7.0,
    '10mm腐蚀': 12.0,
...
def parse_target_value(key, raw):
    ...
    if key == 'T弯' and _is_num(raw):
        return 5.0 if float(raw) == 0.0 else float(raw)  # '0腐蚀'≈5，与字符串口径一致
    if _is_num(raw):
        return float(raw)
