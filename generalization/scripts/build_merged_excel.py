...
lines = [
    ...
    ('', '  · 有标签样本 440 条（环氧酚醛 345 + 环氧配比方案 68 + 聚酯金黄 27），'
         'T弯实测 354 条 / MEK实测 335 条 / 水煮实测 218 条'),
    ('', '  · 无标签配方 46 条（环氧配比方案 44 + 聚酯金黄 2）'),
    ('', '  · 原料主数据 80 种（同物异名已合并；49 种描述符已按供应商 TDS/SDS 逐字段实测替换）'),
    ('', ''),
    ('二、数据来源', '1. 配料测试数据汇总V1.xlsx（有标签-环氧酚醛）'),
    ('', '2. AI研发26.7.22配比方案.xlsx（环氧-配比方案：T弯取自 T弯G 冲击-硫酸铜腐蚀判定，'
         '开区间档位按配方脆性赋值）'),
    ('', '3. 聚酯金黄-AI(1).xlsx（聚酯：T弯/MEK/水煮）'),
    ...
    ('', '3. 性能结果：实测样本的性能数据（含环氧配比方案 T弯 66 条、聚酯金黄 T弯/MEK/水煮 55 条补全）。'),
    ('', '4. 工艺条件：烘烤温度/时间等。'),
    ...
    ('', '3. 聚酯金黄 T弯/MEK/水煮 已按原表逐 sheet 采集；水煮 27 条实测多集中在 2 级区间，'
         '该体系对水煮判别信息有限，评估时避免常量层抬高综合准确率。'),
    ('', '4. 聚酯金黄 29 条与环氧配比方案 112 条配方无烘烤工艺记录，「配方级机理特征」表中'
         '固化类特征（烘烤依赖）对该部分样本留空，表示不可知而非零固化。'),
    ('', '5. 标为「类别典型值」的原料按 TDS-SDS/ 补档清单逐条替换（牌号未识别的树脂集中在聚酯金黄与配比方案）；「待确认」行（DMP、209-基料）需 SDS 核定。'),
