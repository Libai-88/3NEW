...
lines = [
    ...
    ('', '  · 有标签样本 438 条（环氧酚醛 345 + 环氧配比方案 66 + 聚酯金黄 27），'
         'T弯实测 354 条 / MEK实测 335 条 / 水煮实测 216 条'),
    ('', '  · 无标签配方 48 条（环氧配比方案 46 + 聚酯金黄 2）'),
    ('', '  · 原料主数据 80 种（同物异名已合并；49 种描述符已按供应商 TDS/SDS 逐字段实测替换）'),
    ('', ''),
    ('二、数据来源', '1. 配料测试数据汇总V1.xlsx（有标签-环氧酚醛）'),
    ('', '2. AI研发26.7.22配比方案.xlsx（环氧-配比方案：T弯由 T弯G 冲击-硫酸铜腐蚀判定折算）'),
    ...
    ('', '2. 环氧配比方案（3NX240913-6C 镀铬）T弯 为 T弯G 冲击-5%硫酸铜腐蚀判定折算（0腐蚀≈5、'
         '<10mm≈7、10mm≈12、15mm≈18、>15mm≈22；点状/2级 等不可量化者留空），'
         '仅提供 T弯，不提供 MEK/水煮；评估须按体系拆分报告。'),
    ('', '3. 聚酯金黄 T弯/MEK/水煮 已按原表逐 sheet 采集；水煮 27 条实测集中在 2 级区间，'
