...
==================================================
- 原料主数据：全部在用原料（同物异名已合并；占位行按公开手册值/名称自证修正，余量标「待确认」）
- 配方明细 / 性能结果 / 工艺条件 / 两级特征表 / 建模输入：由 data/merged_data.pkl 全量生成
  （数据构成条目数在「使用说明」中按当次数据自动统计）
...
# ---- 数据构成统计（供使用说明引用，避免手写数字与数据脱节）
from collections import Counter
SYS_ORDER = ['环氧酚醛', '环氧配比方案', '聚酯金黄']
N_ALL = len(all_samples)
N_LAB = sum(1 for x in all_samples if x.get('标签状态') == '实测')
N_UNLAB = N_ALL - N_LAB
BY_SYS = Counter(x['体系'] for x in all_samples)
LAB_BY_SYS = Counter(x['体系'] for x in all_samples if x.get('标签状态') == '实测')
UNLAB_BY_SYS = Counter(x['体系'] for x in all_samples if x.get('标签状态') != '实测')
TGT_CNT = {k: sum(1 for x in all_samples if x.get(k) is not None)
           for k in ('T弯', 'MEK', '水煮')}
N_MAT = len(full_mat)
N_DET = sum(len(x.get('组分', {})) for x in all_samples)
N_TDS = sum(1 for c, d in full_mat.items() if str(d.get('数据来源', '')).startswith('TDS'))
_fmt_sys = lambda c: ' + '.join(f'{k} {c[k]}' for k in SYS_ORDER if c.get(k))
N_MECH_BLANK = sum(1 for x in all_samples if x.get('烘烤温度') is None)
...
lines = [
    ...
    '',
    '一、数据构成',
    f'本文件将现有全部数据源整理并填入终极版模板 v3，共 {N_ALL} 条配方样本、{N_DET} 行组分记录、{N_MAT} 种原料。',
    f'  · 有标签样本 {N_LAB} 条（{_fmt_sys(LAB_BY_SYS)}）：'
    f"T弯实测 {TGT_CNT['T弯']} 条 / MEK实测 {TGT_CNT['MEK']} 条 / 水煮实测 {TGT_CNT['水煮']} 条",
    f'  · 无标签配方 {N_UNLAB} 条（{_fmt_sys(UNLAB_BY_SYS)}）',
    f'  · 原料主数据 {N_MAT} 种（同物异名已合并；{N_TDS} 种描述符已按供应商 TDS/SDS 逐字段实测替换）',
    '',
    '二、数据来源',
    '1. 7.26/8.6/8.14配料测试汇总（有标签-环氧酚醛；7 月批次 200℃/10min，8 月批次 205℃/17min）。',
    '2. AI研发26.7.22配比方案.xlsx（环氧配比方案：T弯取自 T弯G 冲击-5%硫酸铜腐蚀判定，开区间档位按配方脆性赋值）。',
    '3. 聚酯金黄-AI(1).xlsx（聚酯金黄：T弯/MEK/水煮）。',
    '4. AI项目原料送检、部分实验数据.xlsx（原料信息）。',
    '5. TDS-SDS/ 供应商技术数据表与安全说明书 291 份（原料描述符实测值与档案出处）。',
    '6. R8/R9/R10 系列 56 条（200℃/10min 批次）：沿用数据集既有登记。',
    '',
    '三、工作表说明',
    f'1. 原料主数据：{N_MAT} 种原料描述符；「描述符状态」区分 TDS实测/送检组成/手册值/类别典型值/待确认，'
    '「数据来源」标注档案实测字段数，「备注」给出牌号对应依据与档案文件名。',
    f'2. 配方明细：{N_ALL} 样本的配方长表（每行=一个组分）。',
    f'3. 性能结果：长表，一行=一个样本×目标（含同一配方 10#/14#/18# 线棒与多批复测的分列记录），'
    f"实测记录 T弯 {TGT_CNT['T弯']} / MEK {TGT_CNT['MEK']} / 水煮 {TGT_CNT['水煮']} 条。",
    '4. 工艺条件：烘烤温度/时间/膜厚/基材/批次/线棒号。',
    '5. 配方级描述符：按质量分数加权的原料描述符（线性口径）。',
    '6. 配方级机理特征：当量浓度、化学计量比、交联密度、Fox Tg、固化度、Hansen 距离、PVC 等'
    '非线性机理量（由 workbench/mech_desc.py 计算）。',
    '7. 建模输入：宽表，一行=一个样本，三个目标实测值与特征并列，可直接导入训练。',
    '8. 数据字典：各字段的类型、单位与口径说明。',
    '',
    '四、使用建议',
    '1. 建模时按"标签状态"筛选：实测样本用于训练/验证，无标签样本用于预测。',
    '2. 环氧配比方案（3NX240913-6C 镀铬）T弯 取自 T弯G 冲击-5%硫酸铜腐蚀判定：字面档位'
    '0腐蚀→0、10mm→10、15mm→15；开区间 <10mm 与 >15mm 分别按配方脆性打分差异化赋值'
    '（<10→5~8、>15→16~20，按交联密度排序），点状/2级/有改善 等无法定级者留空。'
    '该协议为镀铬腐蚀判定，评估须按体系拆分报告。',
    '3. 聚酯金黄 T弯/MEK/水煮 已按原表逐 sheet 采集；水煮实测多集中在 2 级区间，'
    '该体系对水煮判别信息有限，评估时避免常量层抬高综合准确率。',
    f'4. 无烘烤记录的样本（{N_MECH_BLANK} 条）在「配方级机理特征」表中固化类特征（烘烤依赖）留空，'
    '表示不可知而非零固化。',
    '5. 标为「类别典型值」的原料按 TDS-SDS/ 补档清单逐条替换（牌号未识别的树脂集中在聚酯金黄与配比方案）；'
    '「待确认」行（DMP、209-基料）需 SDS 核定。',
    '6. 用配套 Windows 工作台可一键完成建模与预测。',
]
r = 1
for text in lines:
...
sys_rows = [
    ...
    ('环氧酚醛','环氧-酚醛缩合','环氧/酚醛','水煮等级','级','越低越好','等级','121℃/60min 水煮，1-4 级'),
    ('环氧配比方案','环氧-酚醛缩合','环氧/酚醛/乙烯基/丙烯酸/聚酯','T弯','mm','越低越好','连续',
     'T弯G冲击-5%硫酸铜*30min 腐蚀判定定档'),
    ('环氧配比方案','环氧-酚醛缩合','环氧/酚醛/乙烯基/丙烯酸/聚酯','MEK擦拭','次','越高越好','计数','MEK溶剂擦拭'),
    ('环氧配比方案','环氧-酚醛缩合','环氧/酚醛/乙烯基/丙烯酸/聚酯','水煮等级','级','越低越好','等级',
     '121℃/60min 杀菌水煮'),
    ('聚酯金黄','羟基-氨基树脂','聚酯/环氧/丙烯酸/乙烯基','T弯','mm','越低越好','连续','杯突/弯曲试验'),
    ('聚酯金黄','羟基-氨基树脂','聚酯/环氧/丙烯酸/乙烯基','MEK擦拭','次','越高越好','计数','MEK溶剂擦拭'),
    ('聚酯金黄','羟基-氨基树脂','聚酯/环氧/丙烯酸/乙烯基','水煮等级','级','越低越好','等级',
     '121℃*60min 水煮，1-4 级'),
]
for row in sys_rows:
    ws.append(list(row))
