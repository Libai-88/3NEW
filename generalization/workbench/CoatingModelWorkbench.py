...
# T弯: sqrt + 噪声掩码(2.49) + keep=96 k=8 w=0.85 + 机理列单调先验 → 掩码内 R²=0.745±0.003（合并版 69 锚定口径重测）
#   （预算扫描 base/mech 在 60~96 于 0.743~0.749 持平；机理列参与全部三目标同一特征空间，
#     其收益在 MEK/水煮上显著、T弯上与基线统计持平，保留以维持跨体系外推路线）
# MEK: 两阶段（边界判别 + 未截尾回归含 p_hi 特征）keep=45 k=1 → 未截尾 R²=0.458±0.004（机理列，NaN 工艺口径，mvp86-P5）
# 水煮分类: keep=80 → 固定 0.5 acc=0.758 / AUC=0.824（机理臂，mvp86-P4；阈值不再折外寻优）
REG_PARAMS = {
    'T弯': dict(
