...
1. **专有原料描述符为估算值**：`workbench/materials.py` 中树脂/助剂等专有原料的描述符为「化学类别典型值 + 产品信息」估算，非 SDS/TDS 实测；85 个原料仅 43 种不同描述符向量（大量专有原料描述符完全相同，化学差异无法在特征层区分）。唯一真实分子描述符来自 `smi_desc.py`（SMILES 计算）。正式应用前需以实测描述符替换。
2. **标签覆盖已扩展、仍聚焦 1 个主力体系**：486 样本中「环氧酚醛」实测标签最全（T弯 277 / MEK 318 / 水煮 189）；「聚酯金黄」已从原始配方/性能表补全 26 条实测标签（T弯 11 / MEK 17 / 水煮 26，见「数据集完善与实验复跑」）；「环氧-配比方案(112)」仍无实测标签。有标签 / 无标签样本在 `w_EEW`（有标签均值 1773 vs 无标签均值 470）、`w_Tg`、`xlink_frac` 上系统性偏移，属真实化学域差异而非录入错误。跨体系泛化与 LOSO 仍因此无法在当前数据上完整验证。
3. **MEK 截尾口径**：按域建议约 100 次即可满足买家需求，300 为退让上限；仅记录值恰为 300 视为右截尾，>300（如 350/400/550）为真实观测值。数据字典与本 README 已统一该口径。
4. **水煮等级口径**：等级 1（最好）~4（最差），员工偶记 5~10 级均属不合格，统一归并为 4。
5. **烘烤条件覆盖有限**：仅有标本体系（环氧酚醛）含有效烘烤温度记录，且分布在 200/205 ℃ 两个值，近乎零方差；其余两体系无烘烤记录。烘烤温度/时间对性能的影响在当前数据上无法充分建模。
6. **无标签体系标签补齐依赖补测**：伪标签 / 推荐测试为半监督 / 主动学习流程（实验 E/S/A）的回填候选状态，当前数据未出现；补齐实测标签后跨体系泛化才能验证。
