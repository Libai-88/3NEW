...
└── generalization/                    # 本目录：跨体系泛化方案（终极版）
    ...
    ├── mechanism-optimization-report.html  # 机理特征工程与评估协议修正报告
...
5. **配方级机理特征层**：在线性加权描述符之上新增 40 维非线性机理量（当量浓度、化学计量比、Flory 交联密度、Fox Tg、Arrhenius 固化度、Hansen 距离、PVC），从化学计量与固化动力学角度补充配方级区分度（实验 P~S）。
...
| 机理特征工程（实验 P~S） | 40 维机理量使 T弯 R² +0.011（公共掩码口径，20 种子）；MEK 未截尾 R² 略降但 AUC +0.003；水煮聚酯金黄 acc→1.0。序回归/r分层/体系加权/工艺缺失指示均无实质增益 |
...
> MEK 口径说明：318 个样本中 43 个实测值恰为 300（真实值 ≥300 未知，视为右截尾），另有 3 个记录到 300 以上（350/400/550）为真实观测值，合计 46 个 ≥300。早期「R²=0.701」为含截尾代理值的虚高口径；实验 K（`scripts/mvp76_experiments.py`、`scripts/mvp76b_experiments.py`）将评估拆分为「未截尾真实 R² + 边界分类准确率」两个诚实指标，工作台 MEK（`workbench/CoatingModelWorkbench.py`）的截尾判别已统一为「仅恰为 300 视为右截尾」，端到端验证与实验 K 一致。

### 数据集完善与实验复跑（最新）

补全性能标签并修复残留缺陷后，对合并版数据集整体重跑（`scripts/mvp74_final_verify.py`，20 种子）：

| 目标 | 历史基线 | 复测·环氧酚醛子集 | 复测·全体系（池化） | 复测·水煮分层 |
| --- | --- | --- | --- | --- |
| T弯 R²（噪声过滤后） | 0.791 (n=251) | **0.7906** (n=245) | 0.782 (n=261) | — |
| MEK 未截尾 R² | 0.495 | **0.545** (n=275) | 0.538 (n=292) | — |
| MEK AFT 边界 acc | 0.9465 | 0.940 | 0.931 | — |
| 水煮 acc（每系列） | 0.804 (n=189) | 0.778 | 0.777 (n=215) | **0.805** (n=215) |

- **数据完善**：从原始配方/性能表补全「聚酯金黄」26 条实测标签；修正 25.9.17/25.9.18 两批被破坏的组成（各样本正确归属单树脂）并重算其后描述符；新增原料描述符补全规则 `workbench/compo_rules.py`。
- **残留 bug 修复**：无工艺记录样本的 `bake_temp/bake_time` 此前为 NaN 污染特征矩阵，现统一归 0（`workbench/CoatingModelWorkbench.py` 的 `_bake_feat`）；MEK 截尾判别统一为「仅恰为 300 视为右截尾」。特征 0 值偏多经核实为结构性 0（配方 one-hot 计数与部分性质恒 0），树模型对其不敏感，保留特征以维持与基线可比。
- **水煮分层建模**：因聚酯金黄（正类率=0）池化会拖低环氧判别，改为按化学体系分而治之（各自特征选择/模型/每系列阈值，再按样本量加权）。**分层综合 acc=0.805，重新达到 0.8 目标**（环氧酚醛 0.778 + 聚酯金黄 1.000——该体系观测中无水煮失败，常数「合格」规则即 100% 正确）。分层脚本 `stratified_water.py` 与本目录报告 `experiment-rerun-report.html` 记录详情。

最新审计与复跑详情见 `experiment-rerun-report.html`；原料信息利用与数据集补全过程见 `composition-completion.html`。

### 机理特征工程与评估协议修正（最新）

在已有线性加权描述符基础上，新增 40 维**配方级机理特征**（`workbench/mech_desc.py`），捕捉原料间非线性化学交互：

- **当量浓度**：环氧/酚羟基/脂肪族羟基/异氰酸酯/胺/催化剂当量（mol/100g），由 EEW/OHV/AV 标准换算
- **化学计量比**：r = 环氧当量/活性氢当量；偏离度 |1−r| 衡量配比与化学计量点的距离
- **交联密度**：Flory-Stockmayer 量级的潜在/有效交联密度 νe（含固化度折扣）
- **Fox 共混 Tg**：1/T 加权的结合料玻璃化转变温度（非线性）
- **Arrhenius 固化度**：以 200℃ 为参考的等效固化时间 t_eff（Ea=90 kJ/mol）
- **Hansen 溶解度距离**：溶剂相与树脂相的 Ra 及最不利配对距离
- **颜料体积浓度 PVC**：按密度换算的颜料/结合料体积比

同步完成的数据质量改进：
- `workbench/handbook_fixes.py`：5 行同物合并（MEK→丁酮等）、8 行改为公开手册值/名称自证值、2 行标注「待确认」
- `scripts/extend_smiles.py`：SMILES 覆盖从 31 扩展到 41 种原料（结构明确的小分子）

**评估协议修正**（`scripts/mvp84_common_mask_eval.py`）：发现此前各特征臂独立做噪声过滤导致评估样本集不一致。改为由 base 臂预测确定公共噪声掩码（|残差| ≤ 2×噪声 std），所有臂在同一掩码、同一评估集上比较。

20 种子终验结果（`scripts/mvp84` + `scripts/mvp85`）：

| 目标 | 臂 | 核心指标 | 机理入选列 | Δ vs base |
| --- | --- | --- | --- | --- |
| T弯（公共掩码） | base | R²=0.8011 (n=261) | 0 | — |
| T弯 | base+mech | R²=0.8077 | 9 | +0.0066 |
| T弯 | base+mech(rec口径) | R²=0.8122 | 10 | +0.0111 |
| MEK 未截尾回归 | base | R²=0.5087 (n=292) | 0 | — |
| MEK | base+mech | R²=0.4790 | 9 | −0.0297 |
| MEK 边界判别 | base | acc=0.9164 / AUC=0.9394 | 0 | — |
| MEK 边界判别 | base+mech | acc=0.9075 / AUC=0.9426 | — | AUC +0.0032 |
| 水煮（二值≥4） | base | acc=0.7953 / AUC=0.8222 | 0 | — |
| 水煮 | base+mech | acc=0.7814 / AUC=0.8237 | 8 | 聚酯金黄 acc 0.96→1.0 |

建模方式改造实验（`scripts/mvp82_modeling_variants.py`）结论：
- **序回归**（水煮）：AUC 略升但工程化输出受限，未采纳
- **r 分箱分层**（水煮）：acc 与系列分层持平，作为备选口径保留
- **体系加权 / 工艺缺失指示**：无增益，不采纳

完整诊断与实验细节见 `mechanism-optimization-report.html`。
...
5. **MEK 截尾口径**：按域建议约 100 次即可满足买家需求，300 为退让上限；仅记录值恰为 300 视为右截尾，>300（如 350/400/550）为真实观测值。数据字典与本 README 已统一该口径。
6. **水煮等级口径**：等级 1（最好）~4（最差），员工偶记 5~10 级均属不合格，统一归并为 4。二值化阈值 ≥4 为现行口径；序回归替代方案见「建模方式改造」一节，未采纳。
7. **烘烤条件覆盖有限**：仅环氧酚醛（345 条中的 345 条有效记录）含烘烤温度/时间，且只分布在 200℃/10min 与 205℃/17min 两个组合；聚酯金黄与配比方案无烘烤记录（聚酯金黄源文件中仅 25.11.11 一批在设计说明里记有"按205*12min"，不足以覆盖该体系）。Arrhenius 等效固化时间等工艺机理量因此只在主力体系上有意义。
8. **无标签体系标签补齐依赖补测**：伪标签 / 推荐测试为半监督 / 主动学习流程（实验 E/S/A）的回填候选状态，当前数据未出现；补齐实测标签后跨体系泛化才能验证。
