...
| 噪声地板（实验 J） | T弯 理论最大 R²=0.789（模型 0.791 已达上限）；MEK 0.966（有空间）；水煮按分类评估 |
...
| MEK 擦拭 | 17.94 | 0.966 | R²=0.701 | 有真实空间，差距来自截尾≥300，改进截尾处理是主攻方向 |
| 水煮等级 | 0.679 | 0.154（回归口径） | acc=0.804（分类） | 离散标签，按分类评估 |

实验 J-3~J-5 验证：加权回归（0.720）、多任务学习（0.689）、机理特征增强（0.691）相对基线（0.691）均无真实提升，**不应作为提分手段**；突破空间在数据质量（降噪）与数据多样性（扩体系）两端。
